...
longnum division_big(longnum a, longnum b, bool &flag){
    int k = 3;
    int length = a.size();
    if (a.size() > b.size())
    {
...
vector<int> normalize1(vector<int> a) {
    int carry = 0;
    for (int &x : a) {
        x += carry;
        carry = x / base;
...
int main()
{
    ...
    while (tests > 0 ) {
        ...
        if (str_operand == '+') {
            ...
            extend_vec(a, n);
            extend_vec(b, n);
            if (max(a.size(), b.size()) < 10000) {
                a = naive_mul(a, b);
            } else {
                a = normalize1(poly_multiply(a, b));
            }
        }
